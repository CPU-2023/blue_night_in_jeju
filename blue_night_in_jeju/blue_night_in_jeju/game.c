...
void titleDraw() {
    ...
    gotoxy(x, y++); printf("                                  	              ");
    gotoxy(x, y++); printf("          | ￣￣￣￣￣￣￣￣￣￣￣￣￣￣ |         ");
    gotoxy(x, y++); printf("             지렁이 게임을 즐겨보세요~               ");
    gotoxy(x, y++); printf("          | ＿＿＿＿＿＿＿＿＿＿＿＿＿＿ |         ");
    gotoxy(x, y++); printf("	               \\ (> - <) /                    ");
    gotoxy(x, y++); printf("	                 \\     /                   ");
    gotoxy(x, y++); printf("	                                  ");
...
int start()
{
    ...
    while (1)
    {
        ...
        if (wormHeadPointer->x == 0 || wormHeadPointer->x == FIELD_WIDTH || wormHeadPointer->y == 0 || wormHeadPointer->y == FIELD_HEIGHT)
        {
            int x = 28, y = 10;
            system("cls");
            gotoxy(x, y++); printf(" _______  _______  __   __  _______    _______  __   __  _______  ______ ");
            gotoxy(x, y++); printf("|       ||   _   ||  |_|  ||       |  |       ||  | |  ||       ||    _ |");
            gotoxy(x, y++); printf("|    ___||  |_|  ||       ||    ___|  |   _   ||  |_|  ||    ___||   | ||");
            gotoxy(x, y++); printf("|   | __ |       ||       ||   |___   |  | |  ||       ||   |___ |   |_||_");
            gotoxy(x, y++); printf("|   ||  ||       ||       ||    ___|  |  |_|  ||       ||    ___||    __  |");
            gotoxy(x, y++); printf("|   |_| ||   _   || ||_|| ||   |___   |       | |     | |   |___ |   |  | |");
            gotoxy(x, y++); printf("|_______||__| |__||_|   |_||_______|  |_______|  |___|  |_______||___|  |_|");
            gotoxy(x, y++);
            gotoxy(x, y++); 
            gotoxy(x, y++); 
            printf("                  벽에 부딛혔습니다.");
        ...
        if (score == 600) {
            ...
            int x = 20, y = 8;
             ...
             gotoxy(x, y++); printf("       __    _  _______  _______  ______   ___      _______  _______ ");
             gotoxy(x, y++); printf("      |  |  | ||       ||       ||      | |   |    |       ||       |");
             gotoxy(x, y++); printf("      |   |_| ||   _   ||   _   ||  _    ||   |    |    ___||  _____|");
             gotoxy(x, y++); printf("      |       ||  | |  ||  | |  || | |   ||   |    |   |___ | |_____ ");
             gotoxy(x, y++); printf("      |  _    ||  |_|  ||  |_|  || |_|   ||   |___ |    ___||_____  |");
             gotoxy(x, y++); printf("      | | |   ||       ||       ||       ||       ||   |___  _____| |");
             gotoxy(x, y++); printf("      |_|  |__||_______||_______||______| |_______||_______||_______|");
            
             gotoxy(x, y++); printf("       _______  ______    _______    ______   _______  __    _  _______ ");
             gotoxy(x, y++); printf("      |   _   ||    _ |  |       |  |      | |       ||  |  | ||       |");
             gotoxy(x, y++); printf("      |  |_|  ||   | ||  |    ___|  |  _    ||   _   ||   |_| ||    ___|");
             gotoxy(x, y++); printf("      |       ||   |_||_ |   |___   | | |   ||  | |  ||       ||   |___ ");
             gotoxy(x, y++); printf("      |       ||    __  ||    ___|  | |_|   ||  |_|  ||  _    ||    ___|");
             gotoxy(x, y++); printf("      |   _   ||   |  | ||   |___   |       ||       || | |   ||   |___ ");
             gotoxy(x, y++); printf("      |__| |__||___|  |_||_______|  |______| |_______||_|  |__||_______|");
             gotoxy(x, y++); printf("");
            FreeWormList(wormTailNode);
            FreeItemList(itemNode);
        }
...
>>>>>>> 5984a707824718d4751ea760a076aea13a9b6f43:blue_night_in_jeju/game.c
